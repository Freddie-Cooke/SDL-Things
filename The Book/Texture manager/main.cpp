...
void Game::render() {
	...
	SDL_RenderClear(renderer_); // Clear the renderer with solid color

	// Render images with texturemanager
	TextureManager::Instance()->drawFrame("ani", 0, 0, 96, 64, 1, currentFrame_, renderer_);
	TextureManager::Instance()->drawFrame("ani", 0, 100, 96, 64, 1, currentFrame_, renderer_);
